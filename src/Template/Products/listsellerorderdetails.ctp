...
foreach($order_details as $order_data){
	...
	$user_details['id'] = $order_data->order->user_id;
	$user_details['order_date'] = $order_data->order->order_date;
	$user_details['total_price'] = $order_data->order->total_price;
	$user_details['payment_mode'] = $order_data->order->payment_mode;
	$user_details['shipping_address'] = $order_data->order->shipping_address;
  $shipping_address = $order_data->order->shipping_address;
	$user_details['billing_address'] = $order_data->order->billing_address;
  $billing_address = $order_data->order->billing_address;
	$user_details['order_status'] = $order_data->order->order_status;

	$order_details_data = $order_data->order->order_details;
}
                  ...
                  <div class="ads-list-area">
                       <?php                      
                        foreach($order_details as $order_data){
                          ...
                          $filename = WWW_ROOT.'gear_img/'.$order_data->gear->upload;

                          if(!file_exists($filename) || $order_data->gear->upload == ''){
                             $order_data->gear->upload = 'no-image.png';
                          }
                      ...
                      <div class="row mb-4 pb-4">
                          ...
                          <div class="col-md-8 my-md-0 my-3">
                              ...
                              <p class="mb-0">
                                Buyer Info:
                                <?php echo $order_data->order->user->first_name." ".$order_data->order->user->last_name.",".$order_data->order->user->phone; ?>
                              </p>
                          </div>
                          <div class="col-md-2">
